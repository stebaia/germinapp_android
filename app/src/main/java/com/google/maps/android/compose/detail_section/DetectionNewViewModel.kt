...
class DetectionNewViewModel(
        ...
        viewModelScope.launch {
            withContext(Dispatchers.IO) {
                detectionUseCase.getPhytosanitaries(
                    selectViewModelContract.selected.value!!.coltura.specie.id,
                    {
        ...
        viewModelScope.launch {
            withContext(Dispatchers.IO) {
                ...
                val detection = Detection(
                    contract = selectViewModelContract.selected.value!!,
                    remoteId = null,
                    localId = null,
                    user = loginViewModel.loggedUser.value!!.get(),
                    data = date,
                    startTime = startTime,
                    endTime = endTime,
                    images = images
                        ...
                        .map { Pair(it.first!!.path, it.second) },
                    note = note,
    ...
    class DetectionNewViewModelFactory(
        ...
        override fun <T : ViewModel> create(modelClass: Class<T>): T {
            return if (modelClass.isAssignableFrom(DetectionNewViewModel::class.java)) {
                DetectionNewViewModel(
                    loginViewModel,
                    selectViewModelContract,
                    detectionViewModel,
                    detectionUseCase,
                    app
                ) as T
